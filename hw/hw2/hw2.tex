...
n_samples, n_features = faces.shape
faces_centered = faces - faces.mean(axis=0)
centered_cov = np.cov(faces_centered, rowvar=False)
...
Using the top 40 principal components, we keep most of the variance of the data and by projecting into the span of these vectors, we can expect to have captured most of the significant characteristics of the data. So performing nearest neighbors in this space with smaller dimension could be if not as accurate.
