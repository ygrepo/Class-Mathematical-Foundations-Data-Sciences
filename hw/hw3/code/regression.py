...
def split_and_plot(a, max_temp_train, rain_train, max_temp_val, rain_val, grid):
    ...
    rain_train_less_a = rain_train[np.where(max_temp_train < a)].reshape(-1, 1)
    max_temp_train_greater_a = max_temp_train[np.where(max_temp_train >= a)].reshape(-1, 1)
    rain_train_greater_a = rain_train[np.where(max_temp_train >= a)].reshape(-1, 1)
    max_temp_val_less_a = max_temp_val[np.where(max_temp_val < a)].reshape(-1, 1)
    rain_val_less_a = rain_val[np.where(rain_val < a)].reshape(-1, 1)
    max_temp_val_greater_a = max_temp_val[np.where(max_temp_val >= a)].reshape(-1, 1)
    rain_val_greater_a = rain_val[np.where(rain_val >= a)].reshape(-1, 1)
    ...
    reg_1.fit(max_temp_val_greater_a, rain_val_greater_a)
    # linear_fit_val_rain_greater_a = reg.predict(grid_less_a)
    # reg = LinearRegression()
    # reg.fit(max_temp_val_less_a, rain_val_less_a)
    # linear_fit_val_rain_less_a = reg.predict(grid_less_a)
