...
def split_and_plot(a, max_temp_train, rain_train, max_temp_val, rain_val, grid):
    ...
    train_less_a_error = mean_squared_error(rain_train_less_a, rain_train_less_a_pred)
    ...
    train_greater_a_error = mean_squared_error(rain_train_greater_a, rain_train_greater_a_pred)

    train_n_samples_less_a = rain_train_less_a.shape[0]
    train_sample_greater_a = rain_train_greater_a.shape[0]
    train_error = train_n_samples_less_a * train_less_a_error + train_sample_greater_a * train_greater_a_error
    train_error = train_error / (train_n_samples_less_a + train_sample_greater_a)
    print("Train:{},{}, {}".format(train_less_a_error, train_greater_a_error, train_error))
    ...
    val_less_a_error = mean_squared_error(rain_val_less_a, rain_val_less_a_pred)


    less_a_val_sample_weights = np.array([1. / rain_val_less_a_pred.shape[0]])
    less_a_val_sample_weights = np.tile(less_a_val_sample_weights, rain_val_less_a_pred.shape[0])
    ...
    val_greater_a_error = mean_squared_error(rain_val_greater_a, rain_val_greater_a_pred)

    val_n_samples_less_a = rain_val_less_a.shape[0]
    val_n_samples_greater_a = rain_val_greater_a.shape[0]
    val_error = val_n_samples_less_a * val_less_a_error + val_n_samples_greater_a * val_greater_a_error
    val_error = val_error / (val_n_samples_less_a + val_n_samples_greater_a)
    print("val:{},{}, {}".format(val_less_a_error, val_greater_a_error, val_error))

    # greater_a_val_sample_weights = np.array([1. / rain_val_greater_a_pred.shape[0]])
    # greater_a_val_sample_weights = np.tile(greater_a_val_sample_weights, rain_val_greater_a_pred.shape[0])
    #
    # stacked_rain_val = np.hstack((rain_val_less_a, rain_val_greater_a))
    # stacked_rain_val_pred = np.hstack((rain_val_less_a_pred, rain_val_greater_a_pred))
    # stacked_val_sample_weights = np.hstack((less_a_val_sample_weights, greater_a_val_sample_weights))
    # val_error = mean_squared_error(stacked_rain_val, stacked_rain_val_pred, stacked_val_sample_weights)
...
def main():
    ...
    a = 4  # best value of a from grid search above
