...
It also  adds a fixed constant $0.05$ to the true $\beta_{\op{true}}$ coefficient which could be significant compared to
 $\beta_{\op{true}}$.However this could be inferred and corrected during the validation step. 
...
when $\lambda \rightarrow 0, \beta_{\op{RR}} \rightarrow (\frac{\beta_{\op{OLS}}} {2} + 0.05) [ 1 \; 1]^T$ and when $\lambda \rightarrow \infty, \beta_{\op{RR}} \rightarrow [0 \; 0]^T$. 
%The ridge regression estimator with the regularization parameter, $\lambda$,
% governs the trade-off between a model that has low bias and a model which reduces the test error (low variance). 
 ...
 \end{align*}    
  ...
  Compared to $y_{\op{OLS}}$, the parameter $\lambda$ allows to control the amount of linearity between the response and control variable (when $\lambda=0, y_{\op{RR}} = y_{\op{OLS}}$), which might be desirable if the test data points, not known in advance, are not totally dependent on the feature $w_1$.
...
\begin{align*}
	\E[\beta_{\op{RR}} ]		&=	(U S^2 U^T + \lambda U \; U^T)^{-1}	(U S^2 U^T \beta_{\op{true}}  + \lambda \beta_{\op{prior}}) \\
						...
						&= 	\frac{S^2  \beta_{\op{true}}  +  \lambda \beta_{\op{prior}}} {(S^2 + \lambda I)^{-1}}
\end{align*} 
...
\begin{align*}
	\Var(\beta_{\op{RR}})	&=	\Var( (X X^T  +  \lambda I)^{-1} (X y + \lambda \beta_{\op{prior}})) \\
						...
						&= 	\sigma^2 	 \text{diag}_{i=1}^p \bigg( \frac{s_i^2} {(s_i^2 + \lambda)^2} \bigg) 	
\end{align*}					
...
\end{align*}
 Note that we find the same limiting behaviors with the regularization parameter $\lambda$ (and number of iterations $k$), that we found for the ridge regression estimator in part (b). 
